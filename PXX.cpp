...
void USART_Init(long baud)
{
    ...
    UCSR2C = (1<<UMSEL01)|(1<<UMSEL00)|(1<<UDORD0);
...
void PXX_Class::prepare(int16_t channels[16])
{
    ...
    for (int i=0; i<8; i++)
    {
        chan = channels[(sendUpperChannels ? (8 + i) : i)] + 32;
        if(sendUpperChannels) {
          chan += PXX_UPPER_LOW;
          //chan += PXX_LOWER_HIGH;
          //chan = chan << 1;
        }
//        uint16_t channelPPM = channels[(sendUpperChannels ? (8 + i) : i)];
